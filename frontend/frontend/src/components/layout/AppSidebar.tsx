import {
  LayoutDashboard, ScanFace, FileCheck, Fingerprint, CreditCard, Settings, Shield,
  Store, ShieldCheck, MonitorSmartphone, KeyRound,
} from "lucide-react";
...
/**
 * Navigation is driven entirely by the server-assigned role.
 *
 * admin        — full access: all modules + admin tools + developer tools
 * kyc_approver — KYC queue + biometric + own personal pages
...
function getNavSections(role: string): { label: string; items: NavItem[] }[] {
  ...
  const kycAdmin: NavItem[] = [
    { title: "Biometric",      url: "/biometric", icon: ScanFace },
    { title: "SSO Federation", url: "/sso",       icon: KeyRound },
  ];
  ...
  switch (role) {
    case "admin":
      return [
        ...
        { label: "Administration", items: kycAdmin },
    ...
    case "kyc_approver":
      return [
        { label: "Modules",   items: core },
        { label: "KYC Admin", items: kycAdmin },
